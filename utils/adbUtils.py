...
# 判断是否设置环境变量ANDROID_HOME,以拼接adb和aapt命令
if "ANDROID_HOME" in os.environ:
    if system == "Windows":
        command = os.path.join(
            ...
            "platform-tools",
            "adb.exe")
        #遍历ANDORID_HOME目录下build-tools所有的aapt.exe文件，取最新一个
        dir = os.path.join(
            os.environ["ANDROID_HOME"],
            "build-tools/")
        specify_str = 'aapt.exe'
        results = []
        folders = [dir]
        ...
        for folder in folders :
            ...
            results += [os.path.join(folder, x) \
                for x in os.listdir(folder) \
                if os.path.isfile(os.path.join(folder, x)) and specify_str in x]
        aapt_command = results[0]
else:
    raise EnvironmentError(
        "Adb/aapt not found in $ANDROID_HOME path: %s." %
        os.environ["ANDROID_HOME"])
...
class ADB(object):
    ...
    def get_memnum(self, package_name):
        """
        获取当前内存详细值
        """
        try:
            return self.__mem_pss(package_name)/1024
        except:
            return None

    def get_fps(self,package_name):
        """
        获取手机FPS，必须打开手机开发者模式下-GPU呈现模式分析-在adb shell dumpsys gfxinfo中
        """
        result = self.shell('dumpsys gfxinfo %s' %self.get_current_package_name())
        f = [l for l in result.stdout.read().splitlines() if l != ""]
        _begin_id = [i for i, v in enumerate(f) if v == "\tDraw\tPrepare\tProcess\tExecute" or v == "\tDraw\tProcess\tExecute"]
        f_sum = 0
        if len(_begin_id):
            begin_id = _begin_id[-1] + 1
            if "View hierarchy:" in f:
                end_id = f.index("View hierarchy:")
                if begin_id != end_id:
                    _result = f[begin_id:end_id]
                    for v in _result:
                        f_sum = sum([float(i) for i in v.split("\t") if i != ''])
                        return f_sum
                else:
                    return f_sum
            else:
...
if __name__ == "__main__":
    # test = ADB()
    # a = test.aapt("dump badging %s "%"C:/Python27/Lib/site-packages/Automation/resource/app_data/app_package/app.apk")
    # a = list(a.readlines())
    # for i in range(0,len(a)):aapt_command
    #     if 'launchable-activity' in a[i]:
    #         print a[i]
    #         print re.findall(r'\'com\w*.*?\'',a[i])[0]
    # print re.findall(r'\'com\w*.*?\'', a[0])[0]
    print 'test'